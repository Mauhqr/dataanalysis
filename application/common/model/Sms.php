...
class Sms extends Common
{
    ...
    public function send($mobile,$code,$params,$verily)
    {
        if(!$mobile){
        ...
        //如果是登陆注册等的短信，增加校验
        if($code == 'reg' || $code == 'login' || $code== 'veri' || $code == 'receive'){
            $where[] = ['mobile', 'eq', $mobile];
            $where[] = ['code', 'eq', $code];
            $where[] = ['ctime', 'gt', time()-60*5];
...
//        $where[] = ['ip', 'eq', get_client_ip()];
    ...
    private function temp($code,$params){
        ...
        {
            case 'receive':
                $msg = "领取礼卷的验证码是".$params['code']."，请勿告诉他人。";
                break;
            case 'reg':                 //注册
