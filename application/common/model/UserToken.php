...
class UserToken extends Common
{
     ...
     * @return array
     */
    public function setToken($user_id,$platform=2){
        $result = array(
            'status' => false,
            'data' => '',
            'msg' => ''
        );
        $springModel = new SpingMember();
        $userInfo = $springModel->where(array('id'=>$user_id))->find();
        if($userInfo){
            $data['user_id'] = $user_id;
            $data['platform'] = $platform;
            $data['ctime'] = time();
            $data['token'] = $this->algorithm($userInfo['id'],$platform,$data['ctime']);
            $re = $this->save($data);
            if($re >0){
                //删除掉旧的token
    ...
    private function algorithm($user_id,$platform,$createtime){
        return md5(md5($user_id.$platform.$createtime).rand(1,10000));
    }
}